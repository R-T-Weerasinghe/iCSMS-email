...
def get_overdue_datetime(opened_datetime: datetime) -> datetime:
    ...
    """
    if not isinstance(opened_datetime, datetime):
        raise TypeError("opened_datetime should be of type datetime")
    config_doc = collection_configurations.find_one({"id": 1})
    overdue_margin: int = config_doc["overdue_margin_time"]
    overdue_date: datetime = opened_datetime + timedelta(days=overdue_margin)
    return overdue_date


def get_first_response_time(email_list: List[EmailInDB], client_name="client", company_name="agent") -> int | None:
    """
...
def get_avg_response_time(email_list: List[EmailInDB], client_name="client", company_name="agent") -> int | None:
    """

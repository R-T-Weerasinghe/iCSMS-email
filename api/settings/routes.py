...
async def receive_notification_channel_data(noti_channel_data: PostingNotiSendingChannelsRecord, user=Depends(get_current_user)):
        ...
        if await services.check_user_name_notisending(username):
           ...
           new_noti_sending_email_rec = NotiSendingChannelsRecord(user_name=username,is_dashboard_notifications=dashboardChannelChecked, is_email_notifications = emailChannelChecked, noti_sending_emails=notiSendingEmails)
